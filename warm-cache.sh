...
# URL for the sitemap index (passed as an argument)
SITEMAP_INDEX_URL="$1"
...
echo "<p class='log-entry'><span class='timestamp'>[$(TZ=\"America/New_York\" date)]</span> Fetching sitemap index from the public URL: $SITEMAP_INDEX_URL...</p>" >> "$TEMP_LOG_FILE"
...
if [ ! -s "$SITEMAP_LIST" ]; then
  echo "<p class='log-entry error'><span class='timestamp'>[$(TZ=\"America/New_York\" date)]</span> No sitemap URLs found in the sitemap index.</p>" >> "$TEMP_LOG_FILE"
...
echo "<p class='log-entry'><span class='timestamp'>[$(TZ=\"America/New_York\" date)]</span> Fetching page URLs from nested sitemaps...</p>" >> "$TEMP_LOG_FILE"
> "$PAGE_URLS"  # Clear previous page URLs file

while IFS= read -r sitemap_url; do
  echo "<p class='log-entry'><span class='timestamp'>[$(TZ=\"America/New_York\" date)]</span> Processing sitemap: $sitemap_url</p>" >> "$TEMP_LOG_FILE"
  
  # Check if curl command succeeded
  if ! curl -s "$sitemap_url" | grep -oE '<loc>[^<]+</loc>' | sed -e 's/<loc>//g' -e 's|</loc>||g' >> "$PAGE_URLS"; then
    echo "<p class='log-entry error'><span class='timestamp'>[$(TZ=\"America/New_York\" date)]</span> Failed to fetch or process sitemap: $sitemap_url</p>" >> "$TEMP_LOG_FILE"
  fi
done < "$SITEMAP_LIST"

# Check if page URLs were found
if [ ! -s "$PAGE_URLS" ]; then
  echo "<p class='log-entry error'><span class='timestamp'>[$(TZ=\"America/New_York\" date)]</span> No page URLs found in the nested sitemaps.</p>" >> "$TEMP_LOG_FILE"
  rm "$SITEMAP_LIST"
  echo "</div></body></html>" >> "$TEMP_LOG_FILE"
  cat "$TEMP_LOG_FILE" >> "$LOG_FILE"
  rm "$TEMP_LOG_FILE"  # Ensure temp_log.html is removed
...
echo "<p class='log-entry'><span class='timestamp'>[$(TZ=\"America/New_York\" date)]</span> Warming up cache by visiting each URL...</p>" >> "$TEMP_LOG_FILE"
while IFS= read -r url; do
  http_status=$(curl -s -o /dev/null -w "%{http_code}" "$url")
  echo "<p class='log-entry'><span class='timestamp'>[$(TZ=\"America/New_York\" date)]</span> Visiting $url - Status Code: <span class='status'>$http_status</span></p>" >> "$TEMP_LOG_FILE"
...
echo "<p class='log-entry success'><span class='timestamp'>[$(TZ=\"America/New_York\" date)]</span> Cache warming completed.</p>" >> "$TEMP_LOG_FILE"
echo "</div></body></html>" >> "$TEMP_LOG_FILE"
...
if [ -f "$LOG_FILE" ]; then
  ...
  if (( line_count > 2000 )); then
    echo "<p class='log-entry warning'><span class='timestamp'>[$(TZ=\"America/New_York\" date)]</span> Log file exceeded 2000 lines. Truncating...</p>" >> "$TEMP_LOG_FILE"
